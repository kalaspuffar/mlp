...
public class MnistMultiLayerPerceptron {
    ...
    public MnistMultiLayerPerceptron(int hiddenN, int hiddenL) {
    ...
    public void populateInput(byte[] data) {
        for(int i = 0; i < inputN; i++) {
            ...
            */
            ...
            inputNeurons[i] = (data[i] & 0xFF) / 255;
        }
    }

    //trains the network according to our parameters
    public boolean trainNetwork(float teachingStep, float lmse, float momentum, MnistReader mnistReader, int maxEpochs) {
        float mse = 999.0f;
        int epochs = 1;
        float error = 0.0f;
        ...
        while(Math.abs(mse-lmse) > 0.0001f) {
            ...
            mnistReader.reset();
            //for each file
            for(int mnistIdx=0; mnistIdx<mnistReader.size(); mnistIdx++) {

                //first populate the input neurons
                try {
                    populateInput(mnistReader.readNextImage());
                } catch (Exception e) {
                    e.printStackTrace();
                    break;
                }
                target = mnistReader.readNextLabel();
    ...
    public int recallNetwork(byte[] data, int answer) {
        //first populate the input neurons
        populateInput(data);
...
//        System.out.println("The neural network thinks that image " + filename + " with the correct answer "+answer+" represents a \n\r\n\r \t\t----->| " + index + " |<------\t\t \n\r\n\r");
        ...
        System.out.println("The neural network thinks that image with the correct answer "+answer+" represents a " + index);
